...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="tei:div[@type = 'chapter'][not(attribute::frus:doc-dateTime-min)]">
        <xsl:choose>

            <!-- Add or update existing attributes where suitable date is found -->

            <xsl:when
                test=".//tei:dateline[not(ancestor::frus:attachment)][not(ancestor::tei:div[type = 'document'])]//tei:date[(@from or @notBefore or @when) or (@to or @notAfter or @when)]">
                <xsl:variable name="date-min"
                    select="subsequence(.//tei:dateline[not(ancestor::frus:attachment)][not(ancestor::tei:div[type = 'document'])]//tei:date[@from or @notBefore or @when], 1, 1)/(@from, @notBefore, @when)[. ne ''][1]/string()"/>
                <xsl:variable name="date-max"
                    select="subsequence(.//tei:dateline[not(ancestor::frus:attachment)][not(ancestor::tei:div[type = 'document'])]//tei:date[@to or @notAfter or @when], 1, 1)/(@to, @notAfter, @when)[. ne ''][1]/string()"/>
