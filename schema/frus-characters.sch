...
<schema queryBinding="xslt3" xmlns="http://purl.oclc.org/dsdl/schematron">
    ...
    <pattern id="punctuation-orientation-and-placement-checks">
        ...
        <rule context="text()[not(parent::tei:catDesc | parent::tei:rendition)]">
            ...
            <assert id="ocr-transposition-error-l-as-1" role="warn" test="not(matches(., 'l\d|\dl'))"
                >Possible OCR error with number 1 transposed as lower case letter L: [<value-of
                    select="string-join(analyze-string(normalize-space(.), '\S*(?:l\d|\dl)\S*')/fn:match ! ('“' || . || '”'), ', ')"
