...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt3"
    ...
    <pattern id="signed-checks">
        ...
        <rule context="tei:signed//tei:persName[not(ancestor::tei:note)]">
            ...
            <assert
                test="count($following-nodes[normalize-space(.) ne '']) eq 0 or ($following-nodes/self::tei:hi/@rend = 'italic' and $following-nodes/self::tei:lb)"
                id="insert-linebreaks">Any persName elements followed by italicized text should be
                separated by lb elements.</assert>
            <assert test="not(following-sibling::tei:persName)" role="warn" id="persname-multiple">This
